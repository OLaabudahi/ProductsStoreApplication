...
//هذا الكود هو نشاط (Activity) في تطبيق Android يسمى "AddNew_ItemActivity". يتم استدعاء هذا النشاط عند النقر على زر "إضافة عنصر جديد" في التطبيق.
//
//في الأسفل سأشرح الأجزاء الرئيسية في الكود:
//
//في الدالة onCreate():
//
//تتم معالجة العناصر المرئية (مثل الأزرار والحقول النصية ومجموعة الخيارات الاختيارية وغيرها) من خلال استدعاء findViewById() وتعيين المتغيرات المناسبة لكل عنصر.
//يتم إنشاء كائن Toast وكائن قاعدة البيانات.
//يتم التحقق من إذن الوصول إلى التخزين الخارجي وطلب الإذن إذا لم يكن ممنوحًا.
//في دالة onActivityResult():
//
//يتم استدعاءها عند العودة من نشاط آخر. في هذا الحالة، يتم التحقق من نتيجة طلب الصورة.
//إذا تم اختيار صورة بنجاح، يتم تحديث الصورة في ImageButton.
//إذا تم إلغاء اختيار الصورة، يتم عرض رسالة توضح ذلك.
//في دالة onClick() لزر btn_go_home:
//
//يتم إنشاء نية (Intent) للانتقال إلى النشاط MainActivity.
//يتم بدء النشاط الجديد باستخدام startActivity().
//في دالة onClick() لزر btn_image:
//
//يتم إنشاء نية (Intent) لاختيار صورة من المعرض.
//يتم التحقق من إذا ما كان الإذن للوصول إلى التخزين الخارجي ممنوحًا قبل تنفيذ النية.
//يتم بدء النشاط الجديد باستخدام startActivityForResult().
//في دالة onClick() لزر btn_addNewItem:
//
//يتم قراءة البيانات المدخلة من الحقول النصية والخيارات الاختيارية وحقل الصورة.
//يتم التحقق من صحة البيانات المدخلة وتنفيذ الإجراءات المناسبة إذا لم تكن البيانات صحيحة.
//إذا كانت البيانات صحيحة، يتم إنشاء كائن من البيانات المدخلة وحفظه في قاعدة البيانات.
//يتم عرض رسالة توضح نجاح عملية الحفظ أو حدوث خطأ.
//هذا هو نبذة عامة عن الكود المرفق. إذا كان لديك أي أسئلة محددة حول الجزء معين من الكود، فلا تتردد في طرحها.
