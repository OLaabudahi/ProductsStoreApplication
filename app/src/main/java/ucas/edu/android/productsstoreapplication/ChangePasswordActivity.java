...
//هذا الكود هو صفحة ChangePasswordActivity التي تستخدم لتغيير كلمة المرور في التطبيق. هنا هو شرح الكود:
//
//يتم تعريف المتغيرات والعناصر الرسومية المستخدمة في الصفحة مثل ImageButton و EditText و CheckBox و Button و Toast.
//تُعيَّن القيمة المستخدمة لـ SharedPreferences والتحرير الخاص بها.
//يتم تعريف الدالة toastTextChanhe_Show لعرض رسالة توست (Toast) تحتوي على نص محدد.
//يتم تعريف الدالة entering_erorr لإظهار تنسيق خاطئ لحقل الإدخال (EditText) في حالة وجود خطأ في الإدخال.
//يتم تعريف الدالة check_box_is_unCheck لإظهار تنسيق خاطئ لمربع الاختيار (CheckBox) في حالة عدم تحديده.
//يتم تعيين الأحداث النقرية للزر (Button) وأزرار ImageButton للتنقل إلى الشاشة الرئيسية وتنفيذ عملية تغيير كلمة المرور.
//في حالة النقر على الزر "تغيير" (btn_change)، يتم التحقق من صحة البيانات المدخلة ومطابقتها مع كلمة المرور القديمة للمستخدم المسجل. إذا كانت البيانات صحيحة، يتم تحديث كلمة المرور في SharedPreferences ومسح حقول الإدخال وإظهار رسالة توست بنجاح التغيير. إذا كانت البيانات غير صحيحة، يتم عرض رسائل توست لإبلاغ المستخدم بالأخطاء المحتملة.